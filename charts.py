...
def line_chart(values, labels=None, width=940, height=260):
    if not values:
    ...
    if labels:
        ...
        for i in range(0, n, step):
            parts.append(f'<text class="chart-axis" x="{x(i):.1f}" y="{height - 8}" '
                         f'text-anchor="middle">{html.escape(str(labels[i]))}</text>')
    parts.append("</svg>")
    return "".join(parts)


def bar_chart(items, width=940, bar_h=24, gap=12, label_w=160):
    if not items:
        return '<div class="chart-empty">No data yet</div>'
