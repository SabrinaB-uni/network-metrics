...
# Relative activity by hour of day (0-23).
_HOURLY = (
...
def _build_client_pool(n=260):
    rng = random.Random("netwatch-client-pool")
    pool = []
    for i in range(n):
        ...
        if role == "iot":
            ...
            if role == "guest":
                ...
                if vendor == "Apple" and "mac" in os_:
                    kind = "macbook"
                elif os_.startswith("iOS"):
                    kind = "iphone"
                elif os_.startswith("Android"):
                    kind = "android"
                else:
                    kind = "laptop"
        ...
        pool.append({
            "mac": mac, "hostname": hostname, "username": username,
            "access_role": role, "vendor": vendor, "model_os": os_,
        })
    return pool
...
def _factor(when):
    ...
    return lo + (hi - lo) * (when.minute / 60.0)
...
def snapshot(when=None):
    when = when or datetime.now()
    rng = random.Random(int(when.replace(second=0, microsecond=0).timestamp()))
    factor = _factor(when)
    ...
    for idx, (name, loc, floor, model, peak, cap, offline) in enumerate(_ROSTER):
        if offline:
            access_points.append({
                ...
                "status": "Offline", "clients": 0, "load_pct": 0, "uptime_secs": 0,
            })
            continue

        count = min(cap, max(0, round(peak * factor * rng.uniform(0.82, 1.12))))
        ...
        access_points.append({
            ...
            "uptime_secs": int((when - boot).total_seconds()),
        })
        ...
        for _ in range(count):
            ...
            clients.append({
                **dev,
                "ip": f"10.{10 + idx}.{_floor_num(floor)}.{rng.randint(2, 254)}",
                "status": "Connected",
                "ap_name": name,
            })
