"""
The poller: collects a snapshot from the active Aruba client (mock or live)
and writes it to the database. Run it alongside the web app.
...
def poll_once(client=None):
    ...
    try:
        ...
        with db.connection() as conn:
            db.record_poll(conn, getattr(client, "source", "?"), 0, 0,
                           int((time.time() - started) * 1000), False,
                           f"{type(exc).__name__}: {exc}", ts)
